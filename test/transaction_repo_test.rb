...
class TransactionRepositoryTest <  Minitest::Test
  ...
  def test_it_finds_all_by_invoice_id
    transaction = Transaction.new(:invoice_id => 14)
    transaction2 = Transaction.new(:invoice_id => 25)
    transaction3 = Transaction.new(:invoice_id => 25)
    transactions = [transaction, transaction2, transaction3]
    repo = TransactionRepo.new(transactions)
    assert_equal [transaction2, transaction3], repo.find_all_by_invoice_id(25)
  end

  def test_it_finds_all_by_credit_card_number
    transaction = Transaction.new(:credit_card_number => 4654405418249632)
    transaction2 = Transaction.new(:credit_card_number => 4580251236515201)
    transaction3 = Transaction.new(:credit_card_number => 4580251236515201)
    transactions = [transaction, transaction2, transaction3]
    repo = TransactionRepo.new(transactions)
    assert_equal [transaction2, transaction3], repo.find_all_by_credit_card_number(4580251236515201)
  end
